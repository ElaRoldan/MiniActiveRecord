module MiniActiveRecord
  ...
  class Model
    ...
    def self.all
      ...
      MiniActiveRecord::Model.execute("SELECT * FROM #{table_name}s").map do |row|
        self.new(row)
      end
    end
    ...
    def self.where(query, *args)
      table_name = self.to_s
      MiniActiveRecord::Model.execute("SELECT * FROM #{table_name}s WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
  ...
  def insert!
    ...
    insert_sql = "INSERT INTO #{table_name}s (#{fields.join(',')}) VALUES (#{marks})"
  ...
  def update!
    ...
    update_sql = "UPDATE #{table_name}s SET #{update_clause} WHERE id = ?"
